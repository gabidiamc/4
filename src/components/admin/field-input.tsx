...
export function FieldInput({
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      setUploadError("Por favor seleccione un archivo de imagen válido (PNG, JPG, SVG, WebP).");
      return;
    }

    if (file.size > 5 * 1024 * 1024) {
      setUploadError("La imagen es demasiado grande. Seleccione un archivo menor a 5MB.");
      return;
    }
        ...
        {value ? (
          <div className="relative flex items-center gap-4 p-3 rounded-2xl border border-border bg-card/60 shadow-xs">
            ...
            <div className="flex-1 min-w-0 space-y-1">
              <p className="text-xs font-semibold text-foreground truncate">Imagen cargada</p>
              <p className="text-[11px] text-muted-foreground truncate font-mono">
                {String(value).startsWith("data:") ? "Imagen local (Base64)" : String(value)}
              </p>
            </div>
            <Button
              type="button"
